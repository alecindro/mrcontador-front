...
export class ComprovanteComponent implements OnInit, OnDestroy {
  ...
  registerChangeInComprovantes(): void {
    this.eventSubscriber = this.eventManager.subscribe('comprovanteListModification', () => this.loadPage());
  }

  delete(comprovante: IComprovante): void {
    const modalRef = this.modalService.open(ComprovanteDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.comprovante = comprovante;
  }
