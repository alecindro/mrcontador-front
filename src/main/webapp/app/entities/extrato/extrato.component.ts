...
export class ExtratoComponent implements OnInit, OnDestroy {
  ...
  registerChangeInExtratoes(): void {
    this.eventSubscriber = this.eventManager.subscribe('extratoListModification', () => this.loadPage());
  }

  delete(extrato: IExtrato): void {
    const modalRef = this.modalService.open(ExtratoDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.extrato = extrato;
  }
